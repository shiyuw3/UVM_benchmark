...
void usage(int argc, char **argv) {
  ...
  fprintf(stderr, "\t<rows/cols>  - number of rows/cols in the grid "
                  "(positive integer)\n");
  fprintf(stderr, "\t<layers>  - number of layers in the grid "
                  "(positive integer)\n");
...
int main(int argc, char** argv) {
  ...
  hotspot_opt1(powerIn, tempIn, tempOut, numCols, numRows, layers, Cap,
               Rx, Ry, Rz, dt,iterations);

  computeTempCPU(powerIn, tempCopy, answer, numCols, numRows, layers, Cap,
                 Rx, Ry, Rz, dt,iterations);
