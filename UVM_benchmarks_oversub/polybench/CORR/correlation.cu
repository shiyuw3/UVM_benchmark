...
void compareResults(DATA_TYPE* symmat, DATA_TYPE* symmat_outputFromGpu) {
  ...
  for (i = 1; i < (M + 1); i++) {
    for (j = 1; j < (N + 1); j++) {
      if (percentDiff(symmat[i*(N+1) + j], symmat_outputFromGpu[i*(N+1) + j]) >
...
void correlationCuda(DATA_TYPE* data_gpu, DATA_TYPE* mean_gpu,
                     DATA_TYPE* stddev_gpu, DATA_TYPE* symmat_gpu) {
