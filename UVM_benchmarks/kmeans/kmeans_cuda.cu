...
void kmeans(int nreps, int n, int k,
  ...
  for (i = 0; i < nreps; ++i) {
    ...
    recenter_step1<<<1, k>>>(
        sum_x_d, sum_y_d, nx_d, ny_d, x_d, y_d, group_d, n);
    recenter_step2<<<1, k>>>(mu_x_d, mu_y_d, sum_x_d, sum_y_d, nx_d, ny_d);
  }
}

void read_data(float **x, float **y, float **mu_x, float **mu_y, int *n,
               int *k,char* arg);
...
int main(int argc, char* argv[]) {
  ...
  // CUDA_CALL(cudaMemcpy(mu_x_d, mu_x, k*sizeof(float),
  //                      cudaMemcpyHostToDevice));
  // CUDA_CALL(cudaMemcpy(mu_y_d, mu_y, k*sizeof(float),
  //                      cudaMemcpyHostToDevice));
  ...
  std::cerr << "CUDA Took: " << duration.count() << "s" << " for "<< argv[3]
            <<" points." << std::endl;
  gpu_time_used = duration.count();

  /* read back data from gpu */
  // CUDA_CALL(cudaMemcpy(group, group_d, n*sizeof(int),
  //                      cudaMemcpyDeviceToHost));
  // CUDA_CALL(cudaMemcpy(mu_x, mu_x_d, k*sizeof(float),
  //                      cudaMemcpyDeviceToHost));
  // CUDA_CALL(cudaMemcpy(mu_y, mu_y_d, k*sizeof(float),
  //                      cudaMemcpyDeviceToHost));
