...
void usage(int argc, char **argv) {
  fprintf(stderr, "Usage: %s <rows> <cols> <y1> <y2> <x1> <x2> <lamda> "
                   "<no. of iter>\n", argv[0]);
...
void runTest(int argc, char** argv) {
  ...
  for (iter = 0; iter < niter; iter++) {
    ...
    srad_cuda_1<<<dimGrid, dimBlock>>>(
        E_C, W_C, N_C, S_C, J_shared, C_cuda, cols, rows, q0sqr);
    srad_cuda_2<<<dimGrid, dimBlock>>>(
        E_C, W_C, N_C, S_C, J_shared, C_cuda, cols, rows, lambda, q0sqr);
