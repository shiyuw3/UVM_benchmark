...
#define NUM_ASYNCHRONOUS_ITERATIONS 5
...
/**********************************/
void generateRandomGraph(GraphData *graph, int numVertices,
                         int neighborsPerVertex) {
  ...
  gpuErrchk(cudaMallocManaged(&graph->vertexArray,
                              sizeof(int) * graph->numVertices));
  gpuErrchk(cudaMallocManaged(&graph->edgeArray,
                              sizeof(int) * graph->numEdges));
  gpuErrchk(cudaMallocManaged(&graph->weightArray,
...
/************************/
// Finds the vertex with minimum distance value, from the set of vertices not
// yet included in shortest path tree
int minDistance(float *shortestDistances, bool *finalizedVertices,
  ...
  for (int v = 0; v < N; v++) {
    if (finalizedVertices[v] == false && shortestDistances[v] <= min)
      min = shortestDistances[v], minIndex = v;
  }
...
/************************/
void dijkstraCPU(float *graph, float *h_shortestDistances,
                 int sourceVertex, const int N) {
  // h_finalizedVertices[i] is true if vertex i is included in the shortest path
  // tree or the shortest distance from the source node to i is finalized
  bool *h_finalizedVertices = (bool *)malloc(N * sizeof(bool));

  // Initialize h_shortestDistancesances as infinite and h_shortestDistances as
  // false
  for (int i = 0; i < N; i++)
    h_shortestDistances[i] = FLT_MAX, h_finalizedVertices[i] = false;
  ...
  for (int iterCount = 0; iterCount < N - 1; iterCount++) {
    ...
    // processed. currentVertex is always equal to sourceVertex in the first
    // iteration.
    ...
    // Relaxation loop
    for (int v = 0; v < N; v++) {
      // Update dist[v] only if it is not in h_finalizedVertices, there is an
      // edge from u to v, and the cost of the path from the source vertex to v
      // through currentVertex is smaller than the current value of
      // h_shortestDistances[v]
      if (!h_finalizedVertices[v] &&
          graph[currentVertex * N + v] &&
          h_shortestDistances[currentVertex] != FLT_MAX &&
          h_shortestDistances[currentVertex] + graph[currentVertex * N + v] <
              h_shortestDistances[v]) {
        h_shortestDistances[v] =
            h_shortestDistances[currentVertex] + graph[currentVertex * N + v];
      }
...
/***************************/
// Check whether all the vertices have been finalized. This tells the algorithm
// whether it needs to continue running or not.
bool allFinalizedVertices(bool *finalizedVertices, int numVertices) {
  for (int i = 0; i < numVertices; i++) {
    if (finalizedVertices[i] == true) { return false; }
  }
  return true;
}
...
__global__ void initializeArrays(
    ...
    float* __restrict__ d_shortestDistances,
    float* __restrict__ d_updatingShortestDistances,
...
__global__  void Kernel1(const int * __restrict__ vertexArray,
                         ...
                         const float * __restrict__ weightArray,
                         bool * __restrict__ finalizedVertices,
                         float* __restrict__ shortestDistances,
                         float * __restrict__ updatingShortestDistances,
...
/************************/
void dijkstraGPU(GraphData *graph, const int sourceVertex,
                 float * __restrict__ h_shortestDistances) {
  // --- Create device-side adjacency-list, namely, vertex array Va, edge array
  //     Ea and weight array Wa from G(V,E,W)
  // int *d_vertexArray;
  // gpuErrchk(cudaMalloc(&d_vertexArray, sizeof(int) * graph->numVertices));
  // int *d_edgeArray;
  // gpuErrchk(cudaMalloc(&d_edgeArray, sizeof(int) * graph->numEdges));
  // float *d_weightArray;
  // gpuErrchk(cudaMalloc(&d_weightArray, sizeof(float) * graph->numEdges));

  // --- Copy adjacency-list to the device
  // gpuErrchk(cudaMemcpy(d_vertexArray, graph->vertexArray,
  //                      sizeof(int) * graph->numVertices,
  //                      cudaMemcpyHostToDevice));
  // gpuErrchk(cudaMemcpy(d_edgeArray, graph->edgeArray,
  //                      sizeof(int) * graph->numEdges,
  //                      cudaMemcpyHostToDevice));
  // gpuErrchk(cudaMemcpy(d_weightArray, graph->weightArray,
  //                      sizeof(float) * graph->numEdges,
  //                      cudaMemcpyHostToDevice));

  // --- Create mask array Ma, cost array Ca and updating cost array Ua of size
  //     V
  // NOTE: The reason why we call cudaMalloc rather than cudaMallocManaged
  //       is that gpgpu-sim_UVMSmart doesn't simulate UVM in full system level,
  ...
  //     to \u221e
  initializeArrays<<<iDivUp(graph->numVertices, BLOCK_SIZE), BLOCK_SIZE>>>(
      d_finalizedVertices, h_shortestDistances, d_updatingShortestDistances,
      sourceVertex, graph->numVertices);
  ...
  while (!allFinalizedVertices(h_finalizedVertices, graph->numVertices) &&
    ...
    //     iterations than necessary at times, but it will in most cases be
    //     faster because we are doing less stalling of the GPU waiting for
    //     results.
    for (int asyncIter = 0; asyncIter < NUM_ASYNCHRONOUS_ITERATIONS;
         asyncIter++) {
      Kernel1<<<iDivUp(graph->numVertices, BLOCK_SIZE), BLOCK_SIZE>>>(
          graph->vertexArray, graph->edgeArray, graph->weightArray,
          d_finalizedVertices, h_shortestDistances, d_updatingShortestDistances,
          graph->numVertices, graph->numEdges);
      // gpuErrchk(cudaPeekAtLastError());
      // gpuErrchk(cudaDeviceSynchronize());
      Kernel2<<<iDivUp(graph->numVertices, BLOCK_SIZE), BLOCK_SIZE>>>(
          graph->vertexArray, graph->edgeArray, graph->weightArray,
          d_finalizedVertices, h_shortestDistances, d_updatingShortestDistances,
          graph->numVertices);
      // gpuErrchk(cudaPeekAtLastError());
...
int main() {
  ...
  float *weightMatrix =
      (float *)malloc(numVertices * numVertices * sizeof(float));
  ...
  for (int k = 0; k < numVertices; k++) {
    for (int l = 0; l < neighborsPerVertex; l++) {
      weightMatrix[k * numVertices + graph.edgeArray[graph.vertexArray[k] + l]]
          = graph.weightArray[graph.vertexArray[k] + l];
      printf("Vertex nr. %i; Edge nr. %i; Weight = %f\n", k,
             graph.edgeArray[graph.vertexArray[k] + l],
             graph.weightArray[graph.vertexArray[k] + l]);
    }
  }
  ...
  for (int k = 0; k < numVertices; k++) {
    ...
    float *h_shortestDistancesCPU =
        (float *)malloc(numVertices * sizeof(float));
    dijkstraCPU(weightMatrix, h_shortestDistancesCPU, sourceVertex,
                numVertices);
  ...
  //     (float*)malloc(sizeof(float) * graph.numVertices);
  float *h_shortestDistancesGPU;
  gpuErrchk(cudaMallocManaged(&h_shortestDistancesGPU,
                              sizeof(float) * graph.numVertices));
